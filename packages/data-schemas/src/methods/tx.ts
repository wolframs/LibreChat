...
export const tokenValues: Record<string, { prompt: number; completion: number }> = Object.assign(
  {
    ...
    'claude-3.7-sonnet': { prompt: 3, completion: 15 },
    /* Dotted ids (`claude-opus-4.8`) are what inference gateways serve; the hyphenated
       ids are Anthropic's own. Both must be listed: `findMatchingPattern` is a substring
       match, so a dotted id silently falls through to a shorter legacy key otherwise —
       `claude-opus-4.8` would match `claude-opus-4` and bill at 15/75. */
...
export const cacheTokenValues: Record<string, { write: number; read: number }> = {
  ...
  /* Dotted gateway ids — see the note in `tokenValues`; these must be kept in step. */
