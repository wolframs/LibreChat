...
describe('createLoadConfigModels – endpoint profiles', () => {
  const fetchModels = jest.fn();

  /** Admin-configured on both counts: nothing here is `user_provided`, so only
   *  a profile can redirect it. */
  const adminEndpoint = {
    name: 'Gateway',
    baseURL: 'https://admin-default.example.com/v1',
    apiKey: 'sk-admin',
    models: { fetch: true, default: ['fallback-model'] },
  };

  const appConfig = {
    endpoints: { [EModelEndpoint.custom]: [adminEndpoint] },
  };
  ...
  const profileValues = {
    endpointProfiles: {
      active: 'p1',
      profiles: [
        { id: 'p1', name: 'Marketplace', baseURL: 'https://gateway.example.com/v1', apiKey: 'sk-profile' },
      ],
    },
  };
  ...
  beforeEach(() => {
    fetchModels.mockReset().mockResolvedValue(['model-from-profile']);
    mockValidateEndpointURL.mockReset().mockResolvedValue(undefined);
  });

  it('fetches the model list from the active profile, not the configured baseURL', async () => {
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue(appConfig),
      getUserKeyValues: jest.fn().mockResolvedValue(profileValues),
      fetchModels,
    });
    ...
    expect(fetchModels).toHaveBeenCalledTimes(1);
    expect(fetchModels.mock.calls[0][0]).toMatchObject({
      baseURL: 'https://gateway.example.com/v1',
      apiKey: 'sk-profile',
      baseURLIsUserProvided: true,
    });
    expect(result.Gateway).toEqual(['model-from-profile']);
  });

  it('scopes the derived token config to the user', async () => {
    /** Pricing fetched with one user's profile must never be billed against another. */
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue(appConfig),
      getUserKeyValues: jest.fn().mockResolvedValue(profileValues),
      fetchModels,
    ...
    expect(fetchModels.mock.calls[0][0].tokenKey).toBe('Gateway:user-1');
  });

  it('puts the profile URL through the SSRF guard', async () => {
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue(appConfig),
      getUserKeyValues: jest.fn().mockResolvedValue(profileValues),
      fetchModels,
  ...
  it('withholds configured headers from the profile destination', async () => {
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue({
        endpoints: {
          [EModelEndpoint.custom]: [
            { ...adminEndpoint, headers: { Authorization: 'Bearer {{LIBRECHAT_OPENID_ID_TOKEN}}' } },
          ],
        },
      }),
      getUserKeyValues: jest.fn().mockResolvedValue(profileValues),
    ...
    expect(fetchModels.mock.calls[0][0].headers).toBeUndefined();
  });

  it('borrows the endpoint key when the profile carries none', async () => {
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue(appConfig),
      getUserKeyValues: jest.fn().mockResolvedValue({
        endpointProfiles: {
          active: 'p1',
          profiles: [{ id: 'p1', name: 'Marketplace', baseURL: 'https://gateway.example.com/v1' }],
        },
      }),
      fetchModels,
    });
  ...
  it('leaves the shared admin fetch alone when no profile is active', async () => {
    /** The default selection must behave exactly as it did before profiles. */
    const loadConfigModels = createLoadConfigModels({
      getAppConfig: jest.fn().mockResolvedValue(appConfig),
      getUserKeyValues: jest.fn().mockResolvedValue({
        endpointProfiles: { active: 'default', profiles: [profileValues.endpointProfiles.profiles[0]] },
      }),
      fetchModels,
    });
